...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_variables_can_also_be_used_to_access_captures
    ...
    my_own_random_test_1
    my_own_random_test_2
    my_own_random_test_3(nil)
    my_own_random_test_3("steve sun")
  end

  # ------------------------------------------------------------------

  def my_own_random_test_1
    assert_equal "hahaha", "ahahaha"[/(ha)+/]
    if 4%2 == 0
      puts "cool 1..."
    end
  ...
  def my_own_random_test_2
    arr= [3,[4,5]]
    arr= arr.flatten
    array = [1,2].concat(arr)
    puts array
  end

  def my_own_random_test_3(user_name)
    if (user = user_name)
      puts "#{user} does exist"
    else
  ...
  def test_a_vertical_pipe_means_or
    ...
    assert_equal nil, "Jim Gray"[grays, 1] #why? what does this 1 mean?
  end
  ...
  def test_scan_is_like_find_all
    assert_equal ["one", "two", "three"], "one two-three".scan(/\w+/)#what could break dash?
  end
